...
def _get_e_paper_id(session, date):
    ...
    logger.info("epaper.kieler-nachrichten/kieler-nachrichten - status code: %s" % response.status_code)
    assert response.status_code == 200

    current_id = re.findall(r'\d+', re.search('\/webreader\/[0-9]*" class', response.text).group())[0]
...
def _get_e_paper_meta_data(session, current_id):
    ...
    response = session.request("GET", url)
    logger.info("epaper.kieler-nachrichten/webreader-v3 - status code: %s" % response.status_code)
    assert response.status_code == 200
    ...
    assert len(str(articleViewDataUrl)) > 0, "Did not found an articleViewDataUrl for e-paper!"
    ...
    return {"id": id, "articleViewDataUrl": articleViewDataUrl, "releaseDate": releaseDate, "title": title}

def _get_e_paper_article_view(session, articleViewDataUrl):
    url = "https://epaper.kieler-nachrichten.de/webreader-v3/DataLoader.php?uri=" + articleViewDataUrl
    response = session.request("GET", url)
    logger.info("epaper.kieler-nachrichten/webreader-v3/DataLoader - status code: %s" % response.status_code)
    assert response.status_code == 200
...
def download(load_date, secret):
    ...
    with requests.session() as session:
        ...
        article_data = _get_e_paper_article_view(session, meta_dict["articleViewDataUrl"])
        number_of_articles = sum([len(x) for x in article_data])
        logger.info("Found %s pages with %s articles for %s", len(article_data), number_of_articles, load_date.strftime('%d.%m.%Y'))

        data_dir = "./../data/raw/" + load_date.strftime('%d_%m_%Y') + '_' + current_id + '/'
        ...
        for page in range(1, len(article_data) + 1):
            logger.info("Write article data of page %s/%s to file system", page, len(article_data))
            for article in article_data[str(page)]:
                if article['media_type'] == 'article_json':
                    ...
                    logger.debug("Successfully written article data in %s", data_dir + article['articleId'] + '.json')

        check_sum = len(os.listdir(data_dir))
        assert check_sum > 0, "Did not write all articles to file system!"
        logger.info("Successfully wrote %s files with article data to %s", check_sum, data_dir)
