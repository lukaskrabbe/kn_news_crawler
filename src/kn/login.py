...
def login(session, secret):
    ...
    del headers["Cookie"]
    url = (
        "https://epaper.kieler-nachrichten.de/sso-redirect?code"
        "=def50200c019da0f96208b2278d953389d87e6166e7b03fd2dafa5545193347a20"
        "8709498242e39eb41fd7c2d2e459d3fb16b7e529140d4a43bd4869130c4102c75306"
        "e0d7114938df9306f3e7ac435e704d0c42850e6cedc628b52bf1063b0bc09d511c3f"
        "480eeda0b8d2eddc649f8127ab658237967ba832c703bb0f40a8ae51014c78827e0f"
        "a3ac8124b5b07e77f04cad2d916b4b1ae0cb3b6ad98e1de3be9f77c4453fb3283018"
        "051fba1c7db2b309438e23c862cc6acbb6388dc84ab6e750e7c20ada89f4398c6e86"
        "371c0e842f34b2c5d7945ceb56f974aad62a248c170d01a7e811eb17f985f6f9786a"
        "f1324453b37fa39b3da7e1e96a68eac30d71fcf88f8838ad0a434e9f5ee8aff815ef"
        "149a5046e0846e7d4fe1cd3ad57ba03970defd6ff0deece565b3cea70fd57fa04441"
    ...
    response = session.request("GET", url, headers=headers)
    logger.info(
        "epaper.kieler-nachrichten.de/sso-redirect - status code: %s"
        % response.status_code
    )
    assert response.status_code == 200

    url = (
        "https://epaper.kieler-nachrichten.de/kieler-nachrichten/"
        + datetime.today().strftime("%d.%m.%Y")
    )
    response = session.request("GET", url, headers=headers)
    logger.info(
        "epaper.kieler-nachrichten.de/kieler-nachrichten - status code: %s"
        % response.status_code
    )
    assert response.status_code == 200
